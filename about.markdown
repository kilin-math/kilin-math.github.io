---
layout: page
title: About
permalink: /about/
---

I am a mathematican, I work as an Associate professor at Guangdong Technion-Israel Institute of Technology, Shantou, China.

Interests: sandpiles; tropical geometry; analytic/elementary number theory around SL(2,Z), Farey fractions, topographs, and lattice sums; continued fractions; affine geometry. 


[cv](https://kilin-math.github.io/assets/cv.pdf)

![image tooltip here](/assets/me.jpg)


**Recent articles:**

- [Many-point tropical relaxation and the Monge--Ampère equation](https://arxiv.org/abs/2607.25878), with Ernesto Lupercio, Higinio Serrano, and Mikhail Shkolnikov (2026).

- [Residues of a tropical zeta function for convex domains](https://arxiv.org/abs/2604.21709), with Ernesto Lupercio and Mikhail Shkolnikov (2026).

- [Tropical curves in sandpile models](https://doi.org/10.1016/j.aim.2026.111127), with Mikhail Shkolnikov, *Advances in Mathematics* **502** (2026), 111127.

- [Topology of a uniform spanning tree on a cylinder](https://doi.org/10.1088/1402-4896/ae935e), with Denis Rakhmankin, *Physica Scripta* **101** (2026), 325204.

- [Limits of equi-affine equi-distant loci of planar convex domains with two non-parallel asymptotes](https://doi.org/10.3842/SIGMA.2026.071), with Mikhail Shkolnikov, *SIGMA* **22** (2026), 071.

- [Proof of Sun's conjectures on hyperbolic cosine series via the Eisenstein--Lambert method](https://doi.org/10.3934/fcnt.2026018), *Frontiers in Combinatorics and Number Theory* **3** (2026), 21--36.

- [Evaluating lattice sums via telescoping on \(SL_+(2,\mathbb Z)\): a short proof of \(\sum\frac{1}{\|x\|^2\|y\|^2\|x+y\|^2}=\frac{\pi}{4}\) and of Zagier's identity](https://doi.org/10.1142/S1793042126500880), *International Journal of Number Theory* (2026).

- [Legendre duality for certain summations over the Farey pairs](https://arxiv.org/abs/2409.10592).
